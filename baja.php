...
mysql_select_db($database_crm, $crm);
$query_baja = sprintf("SELECT * FROM Tarea AS tr INNER JOIN Cuentas AS ct ON ct.idCuentas=tr.Cuentas_idCuentas  WHERE Asesores_idAsesor=%s AND prioridad LIKE '%%Baja%%' AND Realizada LIKE '%%Programada%%' ORDER BY fechaProgramada ASC", GetSQLValueString($row_usuario['idAsesor'], "text"));
$baja = mysql_query($query_baja, $crm) or die(mysql_error());
...
mysql_select_db($database_crm, $crm);
$query_negativa9 = sprintf("SELECT SUM(montoOportunidad) FROM Oportunidad WHERE fechaActualizacion BETWEEN '".$row_año['YEAR(NOW())']."-10-01' AND '".$row_año['YEAR(NOW())']."-10-30' AND estadoOportunidad LIKE '%%Negativa%%' AND Asesores_idAsesor= %s",GetSQLValueString($row_usuario['idAsesor'], "text"));
...
mysql_select_db($database_crm, $crm);
$query_negativa10 = sprintf("SELECT SUM(montoOportunidad) FROM Oportunidad WHERE fechaActualizacion BETWEEN '".$row_año['YEAR(NOW())']."-11-01' AND '".$row_año['YEAR(NOW())']."-11-31' AND estadoOportunidad LIKE '%%Negativa%%' AND Asesores_idAsesor= %s",GetSQLValueString($row_usuario['idAsesor'], "text"));
            ...
            <div class="box-header">
              ...
              <div class="box-tools pull-right">
                <ul class="pagination pagination-sm inline">
                  ...
                  <li ><a href="inicio.php">Alta</a></li>
                  <li ><a href="media.php	">Media</a></li>
                  <li class="active"><a href="baja.php">Baja</a></li>
              ...
              <ul class="todo-list">
                  ...
                  if($totalRows_baja >0){ 
                    do{
                               ...
                               echo "<span class='text'>La oportunidad <a href='pages/forms/perfiloportunidad.php? oportunidad=".$row_baja['Oportunidad_idOportunidad']."'>".$row_baja['Oportunidad_idOportunidad']."</a> de la empresa <b>".$row_baja['nombreEmpresa']."</b>.Tiene la siguiente actividad pendiente:".$row_baja['actividad'].". Para la fecha";
							   echo "<label class='label label-primary'><i class='fa fa-clock-o'></i>" .$row_baja['fechaProgramada']."</label>";
                               echo "<div class='tools'>";
                               echo "<input type='hidden' name='idTarea' value='".$row_baja['idTarea']."'/>";
                               ...
                               echo "<input type='submit' value='Realizada' class='btn btn-block btn-success btn-xs'>";
							    echo "<a href='pages/forms/cancelaractividad.php?oportunidad=".$row_baja['Oportunidad_idOpotunidad']."'><button type='button' class='btn btn-block btn-danger btn-xs'>Cancelar</button></a></span>";
                              echo "</div>";
							  echo  "</form>"; 
                              echo"</li>";
                    } while ($row_baja = mysql_fetch_assoc($baja));
				  }
				  else
				  {
					echo "<li>";
					echo"<button type='button' class='btn btn-block btn-primary btn-lg ion-plus-circled'> No hay tareas de prioridad baja pendientes</button>";  
				    echo"</li>";
				  }
				?>
