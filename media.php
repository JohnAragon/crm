...
mysql_select_db($database_crm, $crm);
$query_media = sprintf("SELECT * FROM Tarea AS tr INNER JOIN Cuentas AS ct ON ct.idCuentas=tr.Cuentas_idCuentas  WHERE Asesores_idAsesor=%s AND prioridad LIKE '%%Media%%' AND Realizada LIKE '%%Programada%%' ORDER BY fechaProgramada ASC", GetSQLValueString($row_usuario['idAsesor'], "text"));
$media = mysql_query($query_media, $crm) or die(mysql_error());
              ...
              <ul class="todo-list">
                  ...
                  if($totalRows_media >0){ 
                    do{
                               ...
                               echo "<span class='text'>La oportunidad <a href='pages/forms/perfiloportunidad.php? oportunidad=".$row_media['Oportunidad_idOportunidad']."'>".$row_media['Oportunidad_idOportunidad']."</a> de la empresa <b>".$row_media['nombreEmpresa']."</b>.Tiene la siguiente actividad pendiente:".$row_media['actividad'].". Para la fecha";
							   echo "<label class='label label-primary'><i class='fa fa-clock-o'></i>" .$row_media['fechaProgramada']."</label>";
                               echo "<div class='tools'>";
                               echo "<input type='hidden' name='idTarea' value='".$row_media['idTarea']."'/>";
							   ...
							   echo "<a href='pages/forms/cancelaractividad.php?oportunidad=".$row_media['Oportunidad_idOpotunidad']."'><button type='button' class='btn btn-block btn-danger btn-xs'>Cancelar</button></a></span>";
                              echo "</div>";
							  echo  "</form>"; 
                              echo"</li>";
				  ...
				  {
					echo "<li>";
					echo"<button type='button' class='btn btn-block btn-primary btn-lg ion-plus-circled'> No hay tareas de prioridad media pendientes</button>";  
				    echo"</li>";
				  }
				?>
